...
sim_t::sim_t(const cfg_t *cfg, bool halted,
            ...
            std::unordered_map<std::string, adele_params_t> adele_params,
             ...
             std::unordered_map<int, std::vector<std::string>> adele_deactivate) // needed for command line option --cmd
  : htif_t(args),
    cfg(cfg),
    mems(mems),
    ...
    debug_module(this, dm_config)
{
  ...
  for (cpu_offset = fdt_get_first_subnode(fdt, cpu_offset); cpu_offset >= 0;
    ...
    procs.push_back(new processor_t(isa_str, cfg->priv,
                                    ...
                                    log_file.get(), sout_,
                                    adele_params, adele_activate[hartid],
                                    adele_deactivate[hartid]));
